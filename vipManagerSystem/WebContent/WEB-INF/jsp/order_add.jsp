...
<body>
<c:set var="modelList" value="${model}"></c:set>
	<form id="orderForm"
...
<script type="text/javascript">
		...
		model.drink  = str.drink;
		model.food  = str.food;
		model.special  = str.special;
		model.tea  = str.tea;
	...
	function doServlet() {
		...
		if($("#pillType").val()=="01" && parseFloat(bill) > parseFloat(balance)){
			alert("余额不足，请确认会员卡余额");
			return;
		}
		$.ajax({
	...
	function addtr(){
	    ...
	    //添加列内容
	    newEmailTD.innerHTML =  "<select style='width:100px;' name='orderDetailList[" + xuhao + "].productname'' id='product_" + xuhao + "'  onchange='changePrice(this)' >"
		 +"  </select>";
	    //添加列:单价
